...
def channel_shuffle(x, groups):
    batch_size, num_channels, width = x.shape[0:3]
    assert (num_channels % groups == 0,
            'num_channels should be divisible by groups')
    channels_per_group = num_channels // groups
    x = paddle.reshape(
        x=x, shape=[batch_size, groups, channels_per_group, width])
    x = paddle.transpose(x=x, perm=[0, 2, 1, 3])
    x = paddle.reshape(x=x, shape=[batch_size, num_channels, width])
    return x


@BACKBONES.register()
class BMNPP(paddle.nn.Layer):
    """BMN model from
    ...
    def __init__(
        ...
        self.b_conv1_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.b_conv2_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.ts_conv1_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.te_conv1_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.p_conv1_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.p_conv3d1_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.p_conv2d1_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.p_conv2d2_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()
        ...
        self.p_conv2d3_act = paddle.nn.ReLU() if not use_hswish else paddle.nn.Hardswish()

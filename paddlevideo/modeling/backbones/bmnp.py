...
class BMNP(paddle.nn.Layer):
    ...
    def __init__(
        ...
        self.b_conv1_act = paddle.nn.Hardswish()
        ...
        self.b_conv1_1 = paddle.nn.Conv1D(
            ...
            padding=0,
            groups=1,
            weight_attr=init_params('Base_1_1_w', self.hidden_dim_1d, 1),
            bias_attr=init_params('Base_1_1_b', self.hidden_dim_1d, 1))
        self.b_conv1_1_act = paddle.nn.Hardswish()
        ...
        self.b_conv2_act = paddle.nn.Hardswish()
        ...
        self.b_conv2_1 = paddle.nn.Conv1D(
            ...
            padding=0,
            groups=1,
            weight_attr=init_params('Base_2_1_w', self.hidden_dim_1d, 1),
            bias_attr=init_params('Base_2_1_b', self.hidden_dim_1d, 1))
        self.b_conv2_1_act = paddle.nn.Hardswish()

        self.b_conv3 = paddle.nn.Conv1D(
            in_channels=self.hidden_dim_1d,
            out_channels=self.hidden_dim_1d,
            kernel_size=3,
            padding=1,
            groups=4,
            weight_attr=init_params('Base_3_w', self.hidden_dim_1d, 3),
            bias_attr=init_params('Base_3_b', self.hidden_dim_1d, 3))
        self.b_conv3_act = paddle.nn.Hardswish()

        self.b_conv3_1 = paddle.nn.Conv1D(
            in_channels=self.hidden_dim_1d,
            out_channels=self.hidden_dim_1d,
            kernel_size=1,
            padding=0,
            groups=1,
            weight_attr=init_params('Base_3_1_w', self.hidden_dim_1d, 1),
            bias_attr=init_params('Base_3_1_b', self.hidden_dim_1d, 1))
        self.b_conv3_1_act = paddle.nn.Hardswish()
        ...
        self.ts_conv1_act = paddle.nn.Hardswish()

        self.ts_conv1_1 = paddle.nn.Conv1D(
            in_channels=self.hidden_dim_1d,
            out_channels=self.hidden_dim_1d,
            kernel_size=1,
            padding=0,
            groups=1,
            weight_attr=init_params('TEM_s1_1_w', self.hidden_dim_1d, 1),
            bias_attr=init_params('TEM_s1_1_b', self.hidden_dim_1d, 1))
        self.ts_conv1_1_act = paddle.nn.Hardswish()
        ...
        self.ts_conv2 = paddle.nn.Conv1D(
            ...
            padding=1,
            groups=4,
            weight_attr=init_params('TEM_s2_w', self.hidden_dim_1d, 3),
            bias_attr=init_params('TEM_s2_b', self.hidden_dim_1d, 3))
        self.ts_conv2_act = paddle.nn.Hardswish()

        self.ts_conv2_1 = paddle.nn.Conv1D(
            in_channels=self.hidden_dim_1d,
            out_channels=1,
            kernel_size=1,
            padding=0,
            groups=1,
            weight_attr=init_params('TEM_s2_1_w', self.hidden_dim_1d, 1),
            bias_attr=init_params('TEM_s2_1_b', self.hidden_dim_1d, 1))
        self.ts_conv2_1_act = paddle.nn.Sigmoid()
        ...
        self.te_conv1_act = paddle.nn.Hardswish()

        self.te_conv1_1 = paddle.nn.Conv1D(
            in_channels=self.hidden_dim_1d,
            out_channels=self.hidden_dim_1d,
            kernel_size=1,
            padding=0,
            groups=1,
            weight_attr=init_params('TEM_e1_1_w', self.hidden_dim_1d, 1),
            bias_attr=init_params('TEM_e1_1_b', self.hidden_dim_1d, 1))
        self.te_conv1_1_act = paddle.nn.Hardswish()
        ...
        self.te_conv2 = paddle.nn.Conv1D(
            ...
            padding=1,
            groups=4,
            weight_attr=init_params('TEM_e2_w', self.hidden_dim_1d, 3),
            bias_attr=init_params('TEM_e2_b', self.hidden_dim_1d, 3))
        self.te_conv2_act = paddle.nn.Hardswish()

        self.te_conv2_1 = paddle.nn.Conv1D(
            in_channels=self.hidden_dim_1d,
            out_channels=1,
            kernel_size=1,
            padding=0,
            groups=1,
            weight_attr=init_params('TEM_e2_1_w', self.hidden_dim_1d, 1),
            bias_attr=init_params('TEM_e2_1_b', self.hidden_dim_1d, 1))
        self.te_conv2_1_act = paddle.nn.Sigmoid()
        ...
        self.p_conv1_act = paddle.nn.Hardswish()

        self.p_conv1_1 = paddle.nn.Conv1D(
            in_channels=self.hidden_dim_2d,
            out_channels=self.hidden_dim_2d,
            kernel_size=3,
            padding=1,
            groups=1,
            weight_attr=init_params('PEM_1d_1_w', self.hidden_dim_2d, 3),
            bias_attr=init_params('PEM_1d_1_b', self.hidden_dim_2d, 3))
        self.p_conv1_1_act = paddle.nn.Hardswish()
        ...
        self.p_conv3d1_act = paddle.nn.Hardswish()
        ...
        self.p_conv2d1_act = paddle.nn.Hardswish()

        self._se1 = SEModule(self.hidden_dim_2d)

        self.p_conv2d1_2 = paddle.nn.Conv2D(
            in_channels=self.hidden_dim_2d * 2,
            out_channels=self.hidden_dim_2d,
            kernel_size=1,
            stride=1,
            padding=0,
            weight_attr=ParamAttr(name="PEM_2d1_2_w"),
            bias_attr=ParamAttr(name="PEM_2d1_2_b"))
        self.p_conv2d1_2_act = paddle.nn.Hardswish()
        ...
        self.p_conv2d2_act = paddle.nn.Hardswish()

        self._se2 = SEModule(self.hidden_dim_2d)

        self.p_conv2d2_2 = paddle.nn.Conv2D(
            in_channels=self.hidden_dim_2d * 2,
            out_channels=self.hidden_dim_2d,
            kernel_size=1,
            stride=1,
            padding=0,
            weight_attr=ParamAttr(name="PEM_2d2_2_w"),
            bias_attr=ParamAttr(name="PEM_2d2_2_b"))
        self.p_conv2d2_2_act = paddle.nn.Hardswish()
        ...
        self.p_conv2d3_act = paddle.nn.Hardswish()
    ...
    def forward(self, x):
        ...
        x = self.b_conv1_1(x)
        x = self.b_conv1_1_act(x)
        x = self.b_conv2(x)
        x = self.b_conv2_act(x)
        x = self.b_conv2_1(x)
        x = self.b_conv2_1_act(x)
        x = self.b_conv3(x)
        x = self.b_conv3_act(x)
        x = self.b_conv3_1(x)
        x = self.b_conv3_1_act(x)

        #TEM
        xs = self.ts_conv1(x)
        xs = self.ts_conv1_act(xs)
        xs = self.ts_conv1_1(xs)
        xs = self.ts_conv1_1_act(xs)
        xs = self.ts_conv2(xs)
        xs = self.ts_conv2_act(xs)
        xs = self.ts_conv2_1(xs)
        xs = self.ts_conv2_1_act(xs)
        xs = paddle.squeeze(xs, axis=[1])
        xe = self.te_conv1(x)
        xe = self.te_conv1_act(xe)
        xe = self.te_conv1_1(xe)
        xe = self.te_conv1_1_act(xe)
        xe = self.te_conv2(xe)
        xe = self.te_conv2_act(xe)
        xe = self.te_conv2_1(xe)
        xe = self.te_conv2_1_act(xe)
        xe = paddle.squeeze(xe, axis=[1])

        #PEM
        xp = self.p_conv1(x)
        xp = self.p_conv1_act(xp)
        xp = self.p_conv1_1(xp)
        xp = self.p_conv1_1_act(xp)
        #BM layer
        ...
        xp1 = self.p_conv2d1_act(xp)
        xp2 = self._se1(xp)
        xp = paddle.concat([xp1, xp2], axis=1)
        xp = self.p_conv2d1_2(xp)
        xp = self.p_conv2d1_2_act(xp)
        xp = self.p_conv2d2(xp)
        xp1 = self.p_conv2d2_act(xp)
        xp2 = self._se2(xp)
        xp = paddle.concat([xp1, xp2], axis=1)
        xp = self.p_conv2d2_2(xp)
        xp = self.p_conv2d2_2_act(xp)
